...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "List of message", body = [Message]),
    ),
    security(
        ("token" = [])
    )
...
pub(crate) async fn list_message_handler(
    ...
    let msgs = state.list_message(input, id as _).await?;
    Ok(Json(msgs))
}
...
pub(crate) async fn upload_handler(
    ...
    let base_url = &state.config.server.base_dir;
    let mut files = vec![];
...
pub(crate) async fn file_handler(
    ...
    if user.ws_id != ws_id as i64 {
        return Err(AppError::NotFound(
            "File doesn't or you don't have permission".to_string(),
        ));
    }

    let base_url = state.config.server.base_dir.join(ws_id.to_string());
